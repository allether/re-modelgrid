...
var Bar, CELL_PAD, CHAR_W, Grid, GridView, Input, InputCell, MAX_CHAR, MAX_COL_WIDTH, Menu, MenuTab, MoveGuide, MultiGrid, Overlay, Slide, StyleContext, _, cn, css, hotkeys,
...
({Grid} = __webpack_require__(/*! react-virtualized/dist/commonjs/Grid */ "./node_modules/react-virtualized/dist/commonjs/Grid/index.js"));
...
GridView = class GridView extends Component {
  ...
  renderCell(g_opts) {
    var alt_cell, data, data_obj, display_user, doc, e_style, edit_key, fill_fn, g_style, is_key, is_selected, key, key_label, key_name, label_string, lock_icon, max_l, r_color, resize_bar, schema, sort_down, sort_icon, sort_index_label, sort_key, sort_key_index, sort_opts, sort_up, v_w, value;
    boundMethodCheck(this, GridView);
    ...
    if (is_key) {
      ...
      return h('div', {
        ...
        key: g_opts.key
      // sort_index_label
      }, key_label, sort_opts, resize_bar);
    }
    
    // log data_obj.__filled
    display_user = face.getCachedUser(data[g_opts.rowIndex - 1].tracked_user);
    return h('div', {
      style: Object.assign(g_style, g_opts.style, e_style),
      key: g_opts.key
    }, h('div', {
...
var AlertDot, AlertOverlay, Bar, CodeEditor, Color, Component, GridView, HoverBox, Input, JsonView, Menu, MenuTab, ModelGrid, Overlay, QueryBuilderView, QuerySaverView, SearchView, Slide, Style, StyleContext, TabsView, _, cn, createElement, css, generateStyle, highlight, languages, rfc6902,
  boundMethodCheck = function(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new Error('Bound instance method accessed before binding'); } };
...
({Input, MenuTab, Menu, Bar, Overlay, AlertOverlay, StyleContext, AlertDot, HoverBox, Style, generateStyle} = __webpack_require__(/*! re-lui */ "re-lui"));
...
QuerySaverView = __webpack_require__(/*! ./QuerySaverView */ "./components/QuerySaverView.coffee");

ModelGrid = class ModelGrid extends Component {
  constructor(props) {
    super(props);
    this.log = this.log.bind(this);
    this.closeHoverBox = this.closeHoverBox.bind(this);
    ...
    this.setQueryStyle = this.setQueryStyle.bind(this);
    ...
    this.showQuerySaverHoverBox = this.showQuerySaverHoverBox.bind(this);
    ...
    this.g_props = {
      ...
      showQuerySaverHoverBox: this.showQuerySaverHoverBox,
  ...
  async componentDidMount() {
    ...
    // log "RUN QUERY MOUNT",@state.query_item._id
  ...
  componentDidUpdate(props, state) {
    ...
    if (props.schema !== this.props.schema || this.props.schema_data_sync_id !== props.schema_data_sync_id) {
      ...
      this.state.scroll_top = this.state.schema_state_scrolls[this.props.schema.name];
      // log "RUN QUERY UPDATE",@state.query_item._id
      this.runQuery();
    }
  ...
  selectQuery(query_item) {
    ...
    this.runQuery();
    setTimeout(this.saveLocalState, 0);
    this.pushQuery(this.state.query_item);
  ...
  async getSchemaPublicQueries() {
    ...
    this.setSchemaQueryStyles(schema_name);
    return this.setState({});
  }
  ...
  runQuery(run_next) {
    ...
    if (q_i.keyword_input) {
      q_i.json_input = "(" + JSON.stringify(this.getKeywordQueryObject(q_i.keyword_input, q_i)) + ")";
    }
    try {
  ...
  resetQuerySaveState(qi) {
    ...
    qi.color = void 0;
    qi.description = void 0;
    return qi.label = void 0;
  }
  ...
  cloneQuery() {
    ...
    this.resetQueryId(cloned_query);
  ...
  cloneQueryAndSet(edits) {
    var cloned_query;
    boundMethodCheck(this, ModelGrid);
    this.log('cloneQueryAndSet');
    cloned_query = this.cloneQuery();
    this.clearQueryIndex();
    this.state.query_item = cloned_query;
    if (edits) {
      this.editQuery(edits);
    }
    return this.setState({});
  }
  ...
  async saveQuery() {
    var fpriqi, fpubqi, priv_queries, pub_queries, query_item;
    boundMethodCheck(this, ModelGrid);
    ...
    query_item = (await this.props.saveQuery(this.state.schema.name, this.state.query_item));
    // log query_item
    ...
    this.setSchemaQueryStyles(this.state.schema.name);
    this.setState({
      query_item: query_item
    });
    return this.saveLocalState();
  ...
  editQuery(edits) {
    ...
    if (edits.layout_keys || edits.sort_keys) {
      if (this.state.data.get(qi._id) != null) {
        this.state.data.delete(qi._id);
      }
    }
  ...
  mapSchemaQueries(schema_name) {
    var f_q, i, j, len, priv_books, pub_books, queries, query_item, ref, ref1;
...
// @log 'mapSchemaQueries',queries
    for (i = j = 0, len = queries.length; j < len; i = ++j) {
      ...
      if (f_q) {
        if (f_q._v !== query_item._v) {
          this.resetQuerySaveState(query_item);
          this.resetQueryId(query_item);
        } else {
          // @log "SET FOUND QUERY"
          this.state.schema_queries[schema_name][i] = f_q;
          if (((ref = this.state.query_item) != null ? ref._id : void 0) === f_q._id && this.state.query_item._v === f_q._v) {
            this.state.query_item = f_q;
          }
        }
      } else {
        ...
        if (f_q) {
          if (f_q._v !== query_item._v) {
            ...
            // @log "SET FOUND QUERY"
            this.state.schema_queries[schema_name][i] = f_q;
            if (((ref1 = this.state.query_item) != null ? ref1._id : void 0) === f_q._id && this.state.query_item._v === f_q._v) {
              this.state.query_item = f_q;
            }
          }
        } else {
  ...
  setQueryStyle(query_item) {
    ...
    if (query_item.color) {
      ...
      return this.state.query_style_map[query_item._id] = generateStyle({
        step_count: 6,
  ...
  setSchemaQueryStyles(schema_name) {
    var priv_books, pub_books;
    this.state.query_style_map = {};
    priv_books = this.state.private_schema_queries[schema_name];
    pub_books = this.state.public_schema_queries[schema_name];
    if (priv_books != null) {
      priv_books.map(this.setQueryStyle);
    }
    return pub_books != null ? pub_books.map(this.setQueryStyle) : void 0;
  }
  ...
  showQuerySaverHoverBox(bind_el) {
    ...
    return this.setHoverBox({
      ...
      renderContent: (offset_left, offset_top, ref_fn) => {
        return h(QuerySaverView, {
          key: this.state.query_item._id,
          ...
          closeHoverBox: this.closeHoverBox
        });
      },
      getSize: function() {
        return {
  ...
  render() {
    ...
    this.g_props.window_height = this.props.window_height;
    this.g_props.window_width = this.props.window_width;
    ...
    this.g_props.query_style_map = this.state.query_style_map;
    ...
    // beta: @state.show_json_view && 50 || 100
      ...
      query_style_map: this.g_props.query_style_map,
      ref: (el) => {
      ...
      primary: '#2c2e30',
      primary_inv: '#fff',
      secondary: this.context.secondary.color[0],
      secondary_inv: this.context.secondary.inv[0]
...
KeyChip = class KeyChip extends Component {
  ...
  async sortKey(key_name, sort_dir) {
    ...
    k_i = this.props.layout_keys.indexOf(key_name);
  ...
  render() {
    ...
    return h(Draggable, {
      ...
      return h('div', item_props, h('div', {
        ...
        disabled: this.props.layout_keys.length <= 1,
...
QueryBuilderView = class QueryBuilderView extends Component {
  ...
  getInitialState() {
    var ref, ref1;
    return {
      layout_keys: [].concat((ref = this.props.query_item) != null ? ref.layout_keys : void 0),
      sort_keys: _.cloneDeep(((ref1 = this.props.query_item) != null ? ref1.sort_keys : void 0) || []),
      search_key: this.props.query_item.search_key,
      json_input: this.props.query_item.json_input,
      query_type: this.props.query_item.type,
      keyword_input: this.props.query_item.keyword_input,
      edit_v: 0
    };
  }
  ...
  addKey(key_name) {
    boundMethodCheck(this, QueryBuilderView);
    return this.setKeyIndex(key_name, this.state.layout_keys.length);
  }

  editQuery() {
    return this.props.editQuery({
      keyword_input: this.state.keyword_input,
      json_input: this.state.json_input,
      type: this.state.query_type,
      search_key: this.state.search_key,
      layout_keys: this.state.layout_keys,
  ...
  async onClose() {
    boundMethodCheck(this, QueryBuilderView);
    if (!this.props.query_item.updated_at && this.state.edit_v) {
      this.editQuery();
    } else if (this.state.edit_v) {
      await this.props.cloneQueryAndSet();
    }
    if (this.state.edit_v > 0) {
  ...
  saveQuery() {
    ...
    return this.props.closeHoverBox();
  }

  setKeyIndex(key_name, index, sort_dir) {
    var f_i, insert_index, k_i, layout_keys, sort_keys;
    boundMethodCheck(this, QueryBuilderView);
  ...
  onDragEnd(e, n, a) {
    ...
    if (e.destination.droppableId === 'drop-in-sorted') {
      ...
      if (e.source.droppableId === 'drop-in-sorted') {
        return this.setKeyIndex(e.draggableId, e.destination.index, _.find(this.state.sort_keys, {
          key: e.draggableId
        }).dir);
      } else {
  ...
  dropInSortedProvider(provided, snapshot) {
    ...
    return h('div', props, this.state.sort_keys.map((key, i) => {
      var key_name, sort_key_i;
      key_name = key.key;
      sort_key_i = _.findIndex(this.state.sort_keys, {
        key: key_name
      });
      return h(KeyChip, {
        key: key_name + '-sorted-' + String(+this.state.sort_keys[sort_key_i].dir),
        index: i,
        ...
        sort_key: this.state.sort_keys[sort_key_i],
        key_name: key_name
      });
  ...
  dropInProvider(provided, snapshot) {
    ...
    layout_keys = this.state.layout_keys.filter((key_name) => {
      if (!this.props.keys[key_name]) {
        return false;
      }
      if (_.find(this.state.sort_keys, {
        key: key_name
      })) {
        return false;
      }
  ...
  clearQueryInput() {
    ...
    return this.setState({
      ...
      json_error: void 0,
      json_input: void 0,
      keyword_input: void 0
    });
  }

  onSetQueryType(type) {
    boundMethodCheck(this, QueryBuilderView);
    if (this.state.query_type === type) {
      return false;
    }
    if (type === 'json') {
      return this.setState({
        edit_v: this.state.edit_v + 1,
        query_type: 'json',
        json_input: this.state.keyword_input && JSON.stringify(this.getKeywordQueryObject(this.state.keyword_input, this.state.search_key || '_label')),
        keyword_input: void 0
      });
  ...
  render() {
    var clone_query_btn, qi, query_editor, query_title_label, ref, save_query_btn, select_query_key_btn;
    ...
    this.state.unused_keys = this.props.keys_array.filter((key) => {
      return this.state.layout_keys.indexOf(key) < 0;
    });
    if (this.state.query_type === 'keyword') {
    ...
    if (this.state.query_type === 'json') {
      ...
      query_editor = h('div', {
        ...
        value: this.state.keyword_input,
        style: {
          padding: '10px 20px'
        },
        placeholder: 'Search by ' + this.props.schema.keys[this.state.search_key].label
      }));
    }
    if (qi.called_at) {
      clone_query_btn = h(Input, {
        ...
        i: 'file_copy',
        onClick: () => {
          return this.props.cloneQueryAndSet();
        },
        margin_left: false,
        margin_right: true,
        // btn_type: 'true'
    ...
    if (this.state.edit_v && !this.state.json_error && this.props.query_item.label) {
    ...
    query_title_label = h(Input, {
      ...
      btn_type: 'flat',
      big: true,
      i: this.state.query_type === 'json' && 'code' || 'search'
    });
    return h('div', {
      ...
      btn_type: this.state.query_type === 'json' && 'primary' || 'flat'
    }), h(Input, {
      type: 'button',
      i: 'search',
      onClick: this.onSetQueryType.bind(this, 'keyword'),
      btn_type: this.state.query_type === 'keyword' && 'primary' || 'flat'
    })), h('div', {
      ...
      type: 'button',
      i: 'refresh',
      btn_type: !this.state.json_input && !this.state.keyword_input && 'flat' || 'primary',
      disabled: !this.state.json_input && !this.state.keyword_input,
      onClick: this.clearQueryInput
    })), h('div', {
    ...
    }, 'using ' + this.state.query_type), this.state.query_type === 'json' && h('span', {
      className: 'small-mono-fat pad-left',
      style: {
        color: this.state.json_error && this.context.primary.false || this.context.primary.true
      }
    }, ((ref = this.state.json_error) != null ? ref.message : void 0) || 'ok')))));
  }
...
/***/ "./components/QuerySaverView.coffee":
/*!******************************************!*\
  !*** ./components/QuerySaverView.coffee ***!
  \******************************************/
/*! no static exports found */
...
QuerySaverView = class QuerySaverView extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      bookmark_color: this.props.query_item.color,
      bookmark_icon: this.props.query_item.icon,
      edit_v: 0
    };
  }
  ...
  onBookmarkDecriptionInput(e) {
    var v;
    boundMethodCheck(this, QuerySaverView);
    v = e.target.value;
    return this.setState({
      edit_v: this.state.edit_v + 1,
  ...
  render() {
    var bookmark_description, bookmark_icon_input, bookmark_label_input, delete_bookmark_button, qi, save_query_btn;
    qi = this.props.query_item;
    ...
    bookmark_label_input = h(Bar, {
      ...
      type: 'color',
      onInput: this.onSetColor,
      value: this.state.bookmark_color
    }));
    if (qi.updated_at) {
    ...
    bookmark_icon_input = h('div', {
      ...
      btn_type: 'flat',
      onClick: function() {
        return window.open('https://material.io/resources/icons/?style=baseline');
      },
      href: 'https://material.io/resources/icons/?style=baseline'
    }), h(Input, {
      type: 'text',
      i: 'grade',
      value: this.state.bookmark_icon,
    ...
    save_query_btn = h(Input, {
      ...
      margin_right: false,
      type: 'button',
      disabled: !(this.state.edit_v && this.state.bookmark_description && this.state.bookmark_label),
      btn_type: 'true',
    ...
    }, bookmark_label_input, bookmark_icon_input, bookmark_description));
  }
...
var AlertDot, Bar, Input, MAX_CHAR, Menu, MenuTab, SEARCH_BAR_WIDTH, SearchView, Slide, StyleContext, TriangleLoader, cn, css,
  boundMethodCheck = function(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new Error('Bound instance method accessed before binding'); } };

Slide = __webpack_require__(/*! re-slide */ "re-slide");

({AlertDot, Input, MenuTab, Menu, Bar, TriangleLoader, StyleContext} = __webpack_require__(/*! re-lui */ "re-lui"));
...
SearchView = class SearchView extends Component {
  constructor(props) {
    ...
    this.onShowSaveHoverBox = this.onShowSaveHoverBox.bind(this);
    this.showSearchBar = this.showSearchBar.bind(this);
    this.hideSearchBar = this.hideSearchBar.bind(this);
    this.state = {
      search_v: 0,
      search_bar_hidden: true,
      q_v: (ref = props.query_item) != null ? ref._v : void 0,
      search_value: (((ref1 = props.query_item) != null ? ref1.updated_at : void 0) && '/' + props.query_item.label) || ((ref2 = props.query_item) != null ? ref2.keyword_input : void 0) || ""
    };
  }
  ...
  async onSearchEnter() {
    ...
    if (this.state.search_value[0] === '/' && this.state.search_v) {
      ...
      await this.props.selectFirstSearchQuery();
      return this.setState({
        search_value: '/' + this.props.query_item.label
      });
    } else {
  ...
  UNSAFE_componentWillUpdate(props, state) {
    if (this.state.q_v !== props.query_item._v || this.props.query_item !== props.query_item || this.props.mapped_queries_v !== props.mapped_queries_v) {
      if (props.query_item.updated_at) {
        state.search_value = '/' + props.query_item.label;
        return state.q_v = props.query_item._v;
      } else {
  ...
  onShowLayoutHoverBox(e) {
    boundMethodCheck(this, SearchView);
    return this.props.showQueryBuilderHoverBox(this._layout_btn);
  }

  onShowSaveHoverBox(e) {
    boundMethodCheck(this, SearchView);
    return this.props.showQuerySaverHoverBox(this._save_btn);
  }
  ...
  render() {
    ...
    if (qi.error) {
      bar_style = {
        background: this.context.primary.false
      };
      ...
      bar_style = {
        background: this.context.primary.inv[3]
      };
    }
    if (this.state.search_value[0] === '/') {
    ...
    edit_doc_json_button = h('div', {
      className: 'pad2 bot-right'
    }, this.state.search_bar_hidden ? h(Input, {
      type: 'button',
      i: 'visibility',
      className: 'shadow',
    ...
    return h('div', {
      ...
      ref: (el) => {
        if (el) {
          return this._layout_btn = el._outer;
        }
      }
    }), h(Input, {
      type: 'button',
      onClick: this.onShowSaveHoverBox,
      btn_type: this.props.query_item.updated_at && 'true',
      i: this.props.query_item.updated_at && 'bookmark' || 'add',
      big: true,
      ref: (el) => {
        if (el) {
          return this._save_btn = el._outer;
        }
      }
    }))), !this.state.search_bar_hidden ? h('div', {
...
TabsView = class TabsView extends Component {
  ...
  renderTab(qi, index) {
    var btn_style, color_dot, cstyle, f_i, i_color, i_color2, part_spans, qi_user, qi_user_label, ref;
    boundMethodCheck(this, TabsView);
    if (!this._first_query) {
      this._first_query = qi;
    }
    part_spans = [];
    cstyle = this.props.query_style_map[qi._id] || this.context;
    if (!this.state.search_label && qi._id === ((ref = this.props.query_item) != null ? ref._id : void 0) && index !== -1) {
      return h('div', {
    ...
    if (this.state.search_label) {
      f_i = qi.label.indexOf(this.state.search_label);
      if (f_i >= 0) {
        if (f_i > 0) {
        ...
        }, qi.label.substr(f_i, this.state.search_label.length)));
        if (f_i + this.state.search_label.length < qi.label.length) {
          part_spans.push(h('span', {
            key: 'post',
            className: 'pre'
          }, qi.label.substr(f_i + this.state.search_label.length)));
    ...
    if (this.props.query_item._id === qi._id && qi._v === this.props.query_item._v) {
      if (qi.color) {
        btn_style = {
          background: this.state.hover_tab === qi._id && cstyle.primary.inv[1] || cstyle.primary.inv[0],
        ...
        btn_style = {
          ...
          color2: this.context.primary.inv[3],
          width: 260,
          cursor: 'pointer'
        };
      }
      ...
      btn_style = {
        ...
        color: this.context.primary.color[0],
        color2: this.context.primary.color[3],
        width: 260,
        cursor: 'pointer'
      };
    }
    ...
    }, qi.description), qi.is_public && (h('div', {
  ...
  filterAndCombineQueries() {
    ...
    arr = arr.filter((qi, i) => {
      if (!this.state.search_label) {
        ...
        // 	return {_SELECT:qi}
        return true;
      } else if (qi.label.indexOf(this.state.search_label) >= 0) {
        return true;
      }
      return false;
    }).map(this.renderTab);
    if (((ref = this.props.query_item) != null ? ref.updated_at : void 0) && !this.state.search_label) {
      arr.unshift(this.renderSeperator());
      arr.unshift(this.renderTab(this.props.query_item, -1));
    }
    return arr;
  }
  ...
  render() {
    ...
    return h(Slide, {
      ...
      ref: (el) => {
        return this._list = el != null ? el._outer : void 0;
      }
    }, query_tabs);
  }
...
exports.locals = {
	...
	"query-bookmark-description": "lui-g-query-bookmark-description",
