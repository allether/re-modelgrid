...
GridView = class GridView extends Component {
  ...
  toggleSortKey(key, dir) {
    var found_key, keys, n_key;
    boundMethodCheck(this, GridView);
    this.props.query_item.sort_keys = this.props.query_item.sort_keys || [];
    keys = [].concat(this.props.query_item.sort_keys);
    found_key = _.find(keys, {
      key: key
    });
    if (found_key) {
      found_key.dir = dir;
  ...
  columnWidth(g_opts) {
    ...
    boundMethodCheck(this, GridView);
    if (g_opts.index === this.props.query_item.layout_keys.length + 1) {
      return 500;
    }
    if (g_opts.index === 0) {
      return 30;
    ...
    if (this.props.key_col_widths[key._name]) {
      c_w = Math.max(key.min_width || 100, this.props.key_col_widths[key._name]);
    } else {
  ...
  onMoveDone(key, new_width) {
    ...
    this.props.key_col_widths[key._name] = new_width;
    this.setState({
      move_key: null
    });
  ...
  renderCell(g_opts) {
    var alt_cell, data, data_obj, doc, edit_key, fill_fn, g_style, is_key, is_selected, key, key_label, key_name, label_string, lock_icon, max_l, r_color, resize_bar, schema, sort_down, sort_index_label, sort_key, sort_key_index, sort_opts, sort_up, v_w, value;
    boundMethodCheck(this, GridView);
    if (g_opts.columnIndex === this.props.query_item.layout_keys.length + 1) {
      return h('div', {
        key: g_opts.key,
        style: g_opts.style
      });
    ...
    if (g_opts.rowIndex === 0 && g_opts.columnIndex === 0) {
      return h('div', {
        ...
        onClick: this.props.onClearQuerySortKeys
      }, 'clear'));
    }
    if (!is_key && !doc) {
      return h('div', {
    ...
    if (is_key) {
      ...
      if (key.indexed) {
        ...
        sort_opts = h('div', {
          ...
          style: {
            color: (sort_up && 'yellow') || this.context.primary.color[2]
          },
          onClick: this.toggleSortKey.bind(null, key_name, 1),
          className: cn('material-icons', sort_up && css['active'])
        }, 'keyboard_arrow_up'), h('i', {
          style: {
            color: (sort_key && sort_key.dir === -1 && 'cyan') || this.context.primary.color[2]
          },
          onClick: this.toggleSortKey.bind(null, key_name, -1),
      ...
      key_label = h('div', {
        ...
        onClick: this.toggleSortKey.bind(null, key_name, 0)
      }, label_string);
      resize_bar = h('i', {
        style: {
      ...
      return h('div', {
        ...
        style: Object.assign(g_style, g_opts.style),
        key: g_opts.key
      }, sort_index_label, key_label, lock_icon, sort_opts, resize_bar);
    }
  ...
  UNSAFE_componentWillUpdate(props, state) {
    ...
    if (props.data_item !== this.props.data_item) {
      if (props.data_item) {
        state.scroll_to_row = _.findIndex(props.data, {
          _id: props.data_item._id
        });
      } else {
  ...
  componentDidUpdate() {
    ...
    if (this.state.scroll_to_row || this.state.scroll_to_col) {
      return setTimeout(() => {
        return this.setState({
  ...
  onOuterMouseMove(e) {
    ...
    for (key_index = i = 0, len = ref.length; i < len; key_index = ++i) {
      ...
      key = this.props.schema.keys[key_name];
      key_width = this.props.key_col_widths[key._name] || key.col_width;
...
ModelGrid = class ModelGrid extends Component {
  constructor(props) {
    ...
    this.onClearQuerySortKeys = this.onClearQuerySortKeys.bind(this);
    ...
    this.showBookmarksView = this.showBookmarksView.bind(this);
    this.hideRightView = this.hideRightView.bind(this);
    this.showLayoutsView = this.showLayoutsView.bind(this);
  ...
  onClearQuerySortKeys() {
    boundMethodCheck(this, ModelGrid);
    this.state.query_item.sort_keys = [];
  ...
  selectDataItem(item) {
    ...
    return this.props.onSelectDataItem(item);
  }
  ...
  mapQueryItems() {
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      this.state.query_map.set(q._id, q);
      if (((ref1 = this.state.query_item) != null ? ref1._id : void 0) === q._id) {
        this.state.query_item = q;
      }
    }
    return this.setState({
  ...
  resetQueryItemLabel(query_item) {
    ...
    if (is_key) {
      query_item.type = 'key';
      return query_item.input_value = query_item.input_value;
    } else {
  ...
  createQueryItem(query_item) {
    var existing_index, i, j, key, len, qi, ref;
    qi = {
      sort_keys: (query_item != null ? query_item.sort_keys : void 0) || [],
      layout_keys: (query_item != null ? query_item.layout_keys : void 0) || [this.props.schema.default_key || '_label'],
      key: (query_item != null ? query_item.key : void 0) || props.schema.keys_array[0],
      label: query_item != null ? query_item.label : void 0,
      min_date: (query_item != null ? query_item.min_date : void 0) || Date.now() - xDays(365 * 2),
      max_date: (query_item != null ? query_item.max_date : void 0) || Date.now() + xDays(365),
      skip: 0,
    ...
    if (this.props.schema.force_keys) {
      ref = this.props.schema.force_keys;
  ...
  decideQueryItemType(query_item) {
    var ref, ref1;
    if (((ref = query_item.input_value) != null ? ref[0] : void 0) === '{') {
      return query_item.type = 'json';
    } else if (((ref1 = query_item.input_value) != null ? ref1[0] : void 0) === '#') {
  ...
  cleanQuery() {
    ...
    this.state.query_item.layout_keys = this.state.query_item.layout_keys.filter((key) => {
      return this.props.schema.keys[key] != null;
    });
    this.state.query_item.required_keys = this.props.schema.required_keys;
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      key = ref[j];
      this.state.query_item.populate = this.state.query_item.populate.concat(this.props.schema.keys[key].populate || []);
  ...
  runQuery(run_next) {
    ...
    if (run_next === true) {
      ...
      this.state.query_item.limit = 100;
    } else {
      this.state.query_item.limit += this.state.query_item.skip;
  ...
  async runStaticMethod(method) {
    ...
    if (method.run) {
      await method.run(this.props.schema, method);
    } else {
      await this.props.runStaticMethod(this.props.schema, method);
    }
  ...
  async runDataItemMethod(method, callback) {
    ...
    if (method.run) {
      res = (await method.run(this.props.schema, this.state.data_item, method));
    } else {
      res = (await this.props.runDataItemMethod(this.props.schema, this.state.data_item, method));
    }
    this.state.action_query.completed_at = Date.now();
    this.setState({
  ...
  renderStaticMethod(method, get_method_res_callback, method_res) {
    ...
    return method.render(this.props.schema, method_exec, method_res);
  }
  ...
  getDataItem() {
    ...
    return this.props.getDataItem(this.state.data_item._id).then((doc) => {
      ...
      this.state.editor_value_id = null;
      if (this.state.data_item._id === doc._id) {
        this.props.onSelectDataItem(doc);
        return this.setState({
          data_item: doc,
  ...
  componentDidUpdate(props, state) {
    ...
    this.state.scroll_to_index = -1;
    this.state.queries = this.state.queries.slice(0, 5);
    save_state = Object.assign({}, {
      key_col_widths: this.state.key_col_widths,
    ...
    if (props.run_query_item !== this.props.run_query_item) {
      ...
      this.cloneQueryItemAndSet(this.props.run_query_item, this.state.query_item, true);
    }
    if (!this.props.data_item_id && this.state.data_item) {
      this.setState({
        data_item: null
      });
    }
    if (this.props.schema_state_id !== props.schema_state_id || this.props.query_state_id !== props.query_state_id) {
      this.runQuery();
    }
    if (state.queries_updated_at !== this.state.queries_updated_at) {
      this.mapQueryItems();
    }
    if ((!this.state.data_item || state.query_item !== this.state.query_item || !state.data_item) && this.state.show_json_view) {
      this.hideJsonView();
    }
    if (this.state.data_item) {
      if (this.state.data_item._id !== this.state.editor_value_id) {
        if (this.state.data_item) {
          state.editor_value = JSON.stringify(this.state.data_item, null, 4);
          state.editor_patches = [];
        } else {
          state.editor_value = "{}";
          state.editor_patches = [];
        ...
        return this.setState({
          editor_value_id: this.state.data_item._id
        });
      }
    } else {
      if (this.state.editor_value !== '{}' || this.state.editor_patches.length) {
        return this.setState({
          editor_value: '{}',
          editor_patches: []
        });
  ...
  setScrollIndex(state) {
    var data, data_item, i, j, len, results;
    boundMethodCheck(this, ModelGrid);
  ...
  showBookmarksView() {
    boundMethodCheck(this, ModelGrid);
    return this.setState({
      show_bookmarks_view: true,
      show_layouts_view: false
    });
  }
...
SearchView = class SearchView extends Component {
  constructor(props) {
    ...
    this._cell_cache = new CellMeasurerCache({
      defaultHeight: 30,
      fixedWidth: true,
      fixedHeight: false
    });
    ...
    if (!this.props.query_item.called_at && this.props.query_item.type === 'bookmark') {
      if (this.props.query_item.match_label) {
        this.createOrUpdateQueryItem({
          input_value: '#' + this.props.query_item.match_label
        });
      }
    }
    this._cell_cache.clearAll();
    return setTimeout(this.props.onHide, 0);
  }
  ...
  selectItem(query_item) {
    boundMethodCheck(this, SearchView);
    this.props.setQueryItem(query_item, true);
  ...
  mapMenuSearchKeys(key_name, i) {
    ...
    return h(MenuTab, {
      ...
      content: h(Input, {
        ...
        }, h('span', {}, key.label.padEnd(10)), h('span', {
